...
export function GithubLanguages() {
    ...
    const fetchLanguages = async () => {
      try {
        const response = await fetch(
          "https://api.github.com/repos/Areg472/ocean-plus-react/languages",
        );
        if (!response.ok) {
  ...
  return (
    <Card className="mx-auto mb-10 w-[350px] bg-[#0d1117] text-white md:w-[650px] lg:w-[1000px]">
      <CardHeader>
          ...
          <div className="grid gap-2">
            {languagePercentages.map(({ name, percentage }) => (
              <div key={name} className="flex items-center gap-2">
                <span
                  className={`h-3 w-3 rounded-full ${languageColors[name]}`}
                />
